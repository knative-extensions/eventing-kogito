...
CSV_DIR="bundle/manifests"

DEPENDENT_CRDS_KEYS=(knative kogito)
...
getLatestOlmReleaseVersion() {
  ...
  local version=$(cd ${tempfolder}/operators/eventing-kogito && for i in $(ls -d */); do echo ${i%%/}; done | sort -V | tail -1)
  echo ${version}
}

getCsvFile() {
  echo "${CSV_DIR}/eventing-kogito.clusterserviceversion.yaml"
}
