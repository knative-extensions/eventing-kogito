/*
Copyright 2021 The Knative Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by lister-gen. DO NOT EDIT.

package v1alpha1

import (
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
	v1alpha1 "knative.dev/eventing-kogito/pkg/apis/kogito/v1alpha1"
)

// KogitoSourceLister helps list KogitoSources.
// All objects returned here must be treated as read-only.
type KogitoSourceLister interface {
	// List lists all KogitoSources in the indexer.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*v1alpha1.KogitoSource, err error)
	// KogitoSources returns an object that can list and get KogitoSources.
	KogitoSources(namespace string) KogitoSourceNamespaceLister
	KogitoSourceListerExpansion
}

// kogitoSourceLister implements the KogitoSourceLister interface.
type kogitoSourceLister struct {
	indexer cache.Indexer
}

// NewKogitoSourceLister returns a new KogitoSourceLister.
func NewKogitoSourceLister(indexer cache.Indexer) KogitoSourceLister {
	return &kogitoSourceLister{indexer: indexer}
}

// List lists all KogitoSources in the indexer.
func (s *kogitoSourceLister) List(selector labels.Selector) (ret []*v1alpha1.KogitoSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.KogitoSource))
	})
	return ret, err
}

// KogitoSources returns an object that can list and get KogitoSources.
func (s *kogitoSourceLister) KogitoSources(namespace string) KogitoSourceNamespaceLister {
	return kogitoSourceNamespaceLister{indexer: s.indexer, namespace: namespace}
}

// KogitoSourceNamespaceLister helps list and get KogitoSources.
// All objects returned here must be treated as read-only.
type KogitoSourceNamespaceLister interface {
	// List lists all KogitoSources in the indexer for a given namespace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*v1alpha1.KogitoSource, err error)
	// Get retrieves the KogitoSource from the indexer for a given namespace and name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.KogitoSource, error)
	KogitoSourceNamespaceListerExpansion
}

// kogitoSourceNamespaceLister implements the KogitoSourceNamespaceLister
// interface.
type kogitoSourceNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all KogitoSources in the indexer for a given namespace.
func (s kogitoSourceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.KogitoSource, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.KogitoSource))
	})
	return ret, err
}

// Get retrieves the KogitoSource from the indexer for a given namespace and name.
func (s kogitoSourceNamespaceLister) Get(name string) (*v1alpha1.KogitoSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("kogitosource"), name)
	}
	return obj.(*v1alpha1.KogitoSource), nil
}
